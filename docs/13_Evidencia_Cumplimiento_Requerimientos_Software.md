...
La validación se ejecutó mediante **Pruebas de Aceptación de Usuario (UAT - User Acceptance Testing)** y pruebas funcionales de caja negra sobre el entorno de despliegue en producción, evaluando:
...
| :---: | :--- | :--- | :---: | :---: |
| **RF-001** | Catálogo Interactivo | Presentación de prendas con fotos, tallas, precios y stock real en Soles. | Figura 05 | **CUMPLIDO** |
| **RF-002** | Búsqueda y Filtros Dinámicos | Filtrado reactivo en $<100$ ms por nombre, categoría y atributos. | Figura 05 | **CUMPLIDO** |
| **RF-003** | Carrito de Compras Reactivo | Adición, sustracción y recálculo instantáneo de ítems y subtotales. | Figura 06 | **CUMPLIDO** |
| **RF-004** | Validación Automática de Stock | Restricción estricta de selección que impida sobreventas de unidades. | Figura 06 | **CUMPLIDO** |
| **RF-005** | Formulario de Registro de Orden | Captura y validación de cliente, teléfono, dirección y canal. | Figura 07 | **CUMPLIDO** |
| **RF-006** | Liquidación de Flete y Totales | Cálculo aritmético del delivery según destino local vs provincia. | Figura 07 | **CUMPLIDO** |
| **RF-007** | Trazabilidad y Control de Estados | Transición operativa de pedidos (`Recibido`, `Preparación`, `Camino`, etc.). | Figuras 08 y 09 | **CUMPLIDO** |
| **RF-008** | Bandeja de Gestión de Pedidos | Panel centralizado con badges normalizados y filtros multicriterio. | Figura 08 | **CUMPLIDO** |
| **RF-009** | Gestión de Inventario y Precios | Edición de catálogo, ajuste de existencias y precios en tiempo real. | Figura 12 | **CUMPLIDO** |
| **RF-010** | Dashboard de Control Operativo | Monitoreo de ingresos, pedidos en riesgo ($>24$ h) y pipeline diario. | Figura 03 | **CUMPLIDO** |
| **RF-011** | Resumen y Despacho WhatsApp | Enlace formateado listo para envío directo por mensajería al cliente. | Figuras 09 y 10 | **CUMPLIDO** |
| **RF-012** | Control de Acceso y Autenticación | Login seguro de operadores administrativos con credenciales. | Figuras 01 y 02 | **CUMPLIDO** |
| **RF-013** | Despacho Dual (Lima vs Encomienda) | Gestión logística para agencias nacionales (Shalom, Olva) con N° de Guía. | Figuras 07 y 11 | **CUMPLIDO** |
| **RF-014** | Portal de Rastreo Público en Vivo | Consulta de autoservicio del cliente mediante código `LFT-XXX` o DNI. | Figura 13 | **CUMPLIDO** |
| **RF-015** | Captura Estricta de DNI/RUC | Validación obligatoria de documento numérico para envíos interprovinciales. | Figura 07 | **CUMPLIDO** |
| **RF-016** | Medios de Pago y Motor de Cupones | Registro de Yape/Plin/BCP y liquidación de cupones (`LEOFIT10`). | Figura 07 | **CUMPLIDO** |
| **RF-017** | Urgencia de Stock y Garantías | Badges de existencias críticas ($\le 5$ u.) y Garantía LeoFit visible. | Figuras 05, 12 y 13 | **CUMPLIDO** |
| **RF-018** | Recibos PDF y Rótulos Adhesivos | Generación vectorial de comprobante A5 y rótulo de caja imprimible. | Figuras 10 y 11 | **CUMPLIDO** |
...
![Figura 01: Módulo de Autenticación y Control de Acceso](../evidence/requerimientos/01_Login_Autenticacion.png)

*Figura 01: Interfaz de inicio de sesión de LeoFit con control de acceso institucional.*

![Figura 02: Formulario de Autenticación con Credenciales Cargadas](../evidence/requerimientos/02_Login_Credenciales_Llenas.png)

*Figura 02: Carga y verificación reactiva de credenciales del operador (`victor@leofit.com`).*
...
![Figura 03: Dashboard Operativo con Tarjetas Métricas y Pipeline en Tiempo Real](../evidence/requerimientos/03_Dashboard_Control_Operativo.png)

*Figura 03: Panel de mando con KPIs financieros, pedidos en riesgo y distribución porcentual del pipeline.*

![Figura 04: Activación del Modo de Privacidad Granular en Cifras Monetarias](../evidence/requerimientos/04_Dashboard_Privacidad_Oculta.png)

*Figura 04: Ocultamiento instantáneo de montos de ventas para proteger la información en mostradores públicos.*
...
![Figura 05: Catálogo Interactivo con Búsqueda Predictiva de Prendas Deportivas](../evidence/requerimientos/05_Catalogo_Busqueda_Camisetas.png)

*Figura 05: Filtrado dinámico predictivo por texto ('Camiseta') respondiendo en menos de 80 ms.*
...
![Figura 06: Formulario de Registro de Nuevo Pedido y Selector de Prendas](../evidence/requerimientos/06_Nuevo_Pedido_Formulario.png)

*Figura 06: Pantalla de nuevo pedido con selección de artículos, selector de tallas y control de cantidades.*
...
![Figura 07: Liquidación con Despacho por Encomienda, Validación de DNI y Cupón Aplicado](../evidence/requerimientos/07_Nuevo_Pedido_Liquidacion_Cupon_Encomienda.png)

*Figura 07: Registro completo con cliente, teléfono, DNI, dirección, agencia Shalom, flete interprovincial y descuento del 10%.*
...
![Figura 08: Bandeja Centralizada de Gestión de Pedidos con Filtros por Estado](../evidence/requerimientos/08_Bandeja_Gestion_Pedidos.png)

*Figura 08: Vista general de órdenes registradas con buscador, estadísticas rápidas y chips de filtrado.*
...
![Figura 09: Detalle Completo de Orden con Desglose Financiero y Trazabilidad](../evidence/requerimientos/09_Detalle_Pedido_Acordeon.png)

*Figura 09: Acordeón expandido con desglose de prendas, canal, método de pago y acciones operativas.*
...
![Figura 10: Modal de Comprobante Digital Oficial con Descarga PDF e Impresión](../evidence/requerimientos/10_Comprobante_Digital_Recibo.png)

*Figura 10: Recibo digital oficial estructurado conforme a la identidad de marca LeoFit Solutions E.I.R.L.*
...
![Figura 11: Rótulo de Despacho Estandarizado para Agencia de Encomienda (Shalom)](../evidence/requerimientos/11_Comprobante_Rotulo_Encomienda.png)

*Figura 11: Rótulo logístico estandarizado con formato térmico listo para pegar en el embalaje de encomienda.*
...
![Figura 12: Panel de Gestión de Inventario Físico con Alertas de Stock Bajo](../evidence/requerimientos/12_Gestion_Inventario_Stock.png)

*Figura 12: Control de existencias con alertas automáticas de reposición para el taller de confección.*
...
![Figura 13: Portal Público de Rastreo en Vivo con Timeline y Datos de Despacho](../evidence/requerimientos/13_Portal_Rastreo_Timeline.png)

*Figura 13: Rastreo en vivo de orden interprovincial con timeline de 4 fases y datos de courier asignado.*
...
![Figura 14: Modo Accesible de Alto Contraste y Tipografía Agrandada (A11y)](../evidence/requerimientos/14_Modo_Accesible_Alto_Contraste.png)

*Figura 14: Interfaz adaptada con contraste reforzado y escalado ergonómico de fuentes.*
...
| :---: | :--- | :---: | :---: | :---: |
| **RF-001** | Catálogo Interactivo | HU-001 | Figura 05 | Aprobado |
| **RF-002** | Búsqueda y Filtros Dinámicos | HU-001 | Figura 05 | Aprobado |
| **RF-003** | Carrito de Compras Reactivo | HU-002 | Figura 06 | Aprobado |
| **RF-004** | Validación Automática de Stock | HU-002 | Figura 06 | Aprobado |
| **RF-005** | Formulario de Registro de Orden | HU-003 | Figura 07 | Aprobado |
| **RF-006** | Liquidación de Flete y Totales | HU-003 | Figura 07 | Aprobado |
| **RF-007** | Trazabilidad y Control de Estados | HU-004 | Figuras 08 y 09 | Aprobado |
| **RF-008** | Bandeja de Gestión de Pedidos | HU-004 | Figura 08 | Aprobado |
| **RF-009** | Gestión de Existencias y Catálogo | HU-006 | Figura 12 | Aprobado |
| **RF-010** | Dashboard de Control Operativo | HU-005 | Figura 03 | Aprobado |
| **RF-011** | Resumen y Enlace WhatsApp | HU-003 / HU-004 | Figuras 09 y 10 | Aprobado |
| **RF-012** | Control de Acceso Administrativo | HU-007 | Figuras 01 y 02 | Aprobado |
| **RF-013** | Gestión de Despacho Dual (Encomiendas) | HU-003 / HU-004 | Figuras 07 y 11 | Aprobado |
| **RF-014** | Portal de Rastreo Público en Vivo | HU-008 | Figura 13 | Aprobado |
| **RF-015** | Captura Estricta de DNI/RUC | HU-003 | Figura 07 | Aprobado |
| **RF-016** | Métodos de Pago y Cupones Promocionales | HU-003 | Figura 07 | Aprobado |
| **RF-017** | Urgencia de Inventario y Garantías | HU-001 / HU-008 | Figuras 05, 12 y 13 | Aprobado |
| **RF-018** | Emisión de Recibos PDF y Rótulos | HU-003 / HU-004 | Figuras 10 y 11 | Aprobado |
| **RNF-001** | Usabilidad Mobile-First | Transversal | Figuras 06 y 14 | Aprobado |
| **RNF-002** | Rendimiento y Tiempo de Respuesta | Transversal | Figura 05 | Aprobado |
| **RNF-007** | Correlativo Único e Historial | Transversal | Figuras 08 y 09 | Aprobado |
| **RNF-008** | Seguridad y Privacidad Granular | Transversal | Figuras 01 y 04 | Aprobado |
| **RNF-010** | Accesibilidad WCAG 2.1 AAA | Transversal | Figura 14 | Aprobado |
