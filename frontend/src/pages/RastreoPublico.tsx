  ...
  return (
    <div className={`pt-16 pb-24 min-h-screen ${modoAccesible ? "bg-[#E2E8F0]" : "bg-[#F1FAEE]"}`}>
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        {/* Hero de Rastreo */}
        <div className="bg-[#0F223D] rounded-3xl p-6 text-white text-center shadow-xl border border-slate-700">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-[#E63946] to-[#C62828] mb-3 shadow-md">
            <span className="material-icons text-white text-2xl">track_changes</span>
          </div>
          ...
          <form onSubmit={handleBuscar} className="mt-5 flex gap-2 max-w-md mx-auto">
            <input
              ...
              className="flex-1 px-4 py-3.5 bg-white text-slate-900 placeholder-slate-400 rounded-2xl text-base font-semibold focus:outline-none focus:ring-4 focus:ring-red-500/40 shadow-inner"
            />
            <button
              type="submit"
          ...
          <div className="mt-4 flex items-center justify-center gap-2 flex-wrap">
            ...
            {[
              { cod: "LFT-004", label: "Shalom Trujillo (Provincia)" },
              ...
              <button
                ...
                className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white px-2.5 py-1 rounded-xl border border-slate-700 transition-colors font-mono"
              >
                {ej.cod} · {ej.label}
              </button>
            ))}
        ...
        {errorBusqueda && (
          <div className="bg-red-100 border-2 border-red-300 rounded-2xl p-4 flex items-center gap-3 text-red-950">
            <span className="material-icons text-red-700" style={{ fontSize: "24px" }}>error_outline</span>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-200">
              <div className="flex items-start justify-between gap-3 pb-4 border-b border-slate-100 mb-5">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-mono font-bold text-slate-700 bg-slate-100 px-2.5 py-0.5 rounded-lg border border-slate-200">
                      {pedidoEncontrado.numero}
                    </span>
                    <span className="text-xs font-semibold px-2.5 py-0.5 rounded-lg bg-blue-100 text-blue-900 border border-blue-200">
                      {pedidoEncontrado.tipoEnvio === "Nacional" ? "Envío Nacional (Provincia)" : "Envío Local (Lima Capital)"}
                    </span>
                  </div>
                  <h2 className="text-xl font-bold text-slate-900">{pedidoEncontrado.cliente.nombre}</h2>
              ...
              {esCancelado ? (
                <div className="bg-slate-100 border border-slate-300 rounded-2xl p-4 text-center">
                  ...
                  <p className="text-base font-bold text-slate-800">Este pedido fue cancelado</p>
                  <p className="text-xs text-slate-500 font-normal mt-1">Comunícate con soporte si tienes alguna duda.</p>
                </div>
              ) : (
                <div className="relative pl-6 sm:pl-8 space-y-6 my-2">
                  ...
                  {PASOS_ESTADO.map((paso, index) => {
                    ...
                    return (
                      <div key={paso.estado} className="relative flex items-start gap-4">
                        {/* Nodo del timeline */}
                        <div
                              ...
                              {pedidoEncontrado.tipoEnvio === "Nacional" ? (
                                <>
                                  <div className="flex items-center justify-between text-xs font-semibold text-slate-800">
                                    ...
                                    <span className="bg-[#0F223D] text-white px-2.5 py-0.5 rounded-lg font-bold">
                                      {pedidoEncontrado.agenciaEncomienda || "Shalom"}
                                    </span>
                                  </div>
                                  <div className="flex items-center justify-between text-xs text-slate-700">
                                    <span className="font-medium">N° Guía de Remisión:</span>
                                ...
                                <div className="flex items-center justify-between text-xs">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-semibold text-slate-900">Reparto Directo en Lima Capital (Víctor / Motorizado)</span>
                                  </div>
                                </div>
                              )}
                            </div>
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200">
              ...
              <div className="space-y-2 text-sm">
                <div>
                  <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block">Dirección / Agencia:</span>
                  <p className="font-medium text-slate-800">{pedidoEncontrado.cliente.direccion}</p>
                </div>
                {pedidoEncontrado.notas && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-900 font-medium mt-2">
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200">
              ...
              <div className="space-y-2">
                {pedidoEncontrado.items.map((item) => (
                  <div key={item.productoId} className="flex items-center justify-between text-sm py-1 border-b border-slate-100 last:border-none">
                ...
                <div className="pt-2 flex items-center justify-between font-bold text-base text-slate-950">
                  <span>Total Pagado / a Cobrar</span>
                  <span className="font-mono text-xl text-[#E63946]">S/{pedidoEncontrado.total.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-3xl p-5 text-center shadow-sm">
              <p className="text-sm font-bold text-emerald-950 mb-1">¿Tienes dudas o necesitas reprogramar la entrega?</p>
              <p className="text-xs text-emerald-800 font-normal mb-3">Víctor y el equipo de atención están listos para ayudarte.</p>
              <a
                href={`https://wa.me/51987654321?text=${encodeURIComponent(`Hola LeoFit, deseo consultar sobre mi pedido ${pedidoEncontrado.numero}`)}`}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white font-bold px-5 py-3 rounded-2xl text-sm transition-all shadow-md shadow-emerald-700/20"
              >
