    ...
    <div className={`pt-16 pb-24 min-h-screen ${modoAccesible ? "bg-[#E2E8F0]" : "bg-[#F1FAEE]"}`}>
      <div className="max-w-2xl mx-auto px-4 py-5">
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            onClick={abrirNuevo}
            className="flex items-center gap-2 bg-[#E63946] hover:bg-[#C62828] active:scale-95 text-white px-5 py-3 rounded-2xl text-base font-bold transition-all shadow-lg shadow-red-500/30 ring-2 ring-white"
          >
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-100 text-slate-700">
                    ...
                    <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider hidden sm:table-cell">Cat.</th>
                  ...
                  {productos.map((prod, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 hidden sm:table-cell">
                        <span className={`text-xs font-semibold px-3 py-1 rounded-xl uppercase tracking-wide border ${categoriaColor[prod.categoria]}`}>
                          {prod.categoria}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-right">
                        <span className="text-base font-bold font-mono text-slate-900">S/{prod.precio.toFixed(2)}</span>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold border font-mono ${
                          prod.stock <= 5
      ...
      {modalAbierto && (
        <Modal titulo={editando ? "Editar Prenda" : "Nueva Prenda"} onCerrar={() => setModalAbierto(false)}>
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Nombre de Prenda *</label>
              <input
                type="text"
                value={form.nombre}
                onChange={(e) => setForm((f) => ({ ...f, nombre: e.target.value }))}
                placeholder="ej. Camiseta Dry-Fit Pro"
                className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-300 rounded-2xl text-base font-medium text-slate-900 focus:outline-none focus:border-[#0F223D] focus:bg-white"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 border-2 border-slate-300 rounded-2xl text-base font-medium text-slate-900 bg-slate-50"
                >
                  {CATEGORIAS.map((c) => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Talla</label>
                <select
                  value={form.talla}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Color</label>
                <select
                  value={form.color}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Precio (S/) *</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  ...
                  placeholder="0.00"
                  className="w-full px-4 py-3 border-2 border-slate-300 rounded-2xl text-base font-medium font-mono text-slate-900 bg-slate-50"
                />
              </div>
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Stock Disponible *</label>
              <input
                type="number"
                min="0"
                value={form.stock || ""}
            ...
            {formError && (
              <div className="flex items-center gap-2 bg-red-100 border-2 border-red-300 rounded-2xl px-4 py-3">
                ...
                <p className="text-sm text-red-950 font-semibold">{formError}</p>
              </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3.5 border-2 border-slate-300 text-slate-700 font-bold rounded-2xl hover:bg-slate-100 transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={handleGuardar}
                className="flex-1 py-3.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-2xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-emerald-700/30 ring-2 ring-white"
              >
      ...
      {confirmando && (
        <Modal titulo="Confirmar Eliminación" onCerrar={() => setConfirmando(null)}>
          <div className="text-center py-3">
            ...
            <p className="text-base text-slate-600 font-medium mb-1">¿Estás seguro de eliminar esta prenda?</p>
            <p className="text-lg font-bold text-slate-950 mb-6 bg-slate-100 py-2 px-4 rounded-xl border border-slate-200">"{confirmando.nombre}"</p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-2xl transition-colors shadow-lg shadow-red-500/30 ring-2 ring-white"
              >
                Sí, eliminar
              </button>
            </div>
